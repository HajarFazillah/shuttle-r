...
The simulation runs headless by default for best performance. This is a
deliberate choice for this setup: development was done on a machine without
a discrete GPU (software/`llvmpipe` rendering only), where running Gazebo's
GUI continuously made the simulation too slow to iterate on (~0.02-0.03x
real-time, see Performance Notes below). Running headless and only opening
the GUI briefly for verification was the practical workaround.

**If you have a machine with a discrete GPU / more headroom**, you can
launch with the GUI enabled from the start instead of running headless -
the simulation should stay close to real-time, making development and
demoing noticeably smoother. To do this, run Gazebo with the GUI directly
(omit the headless flag in `empty_world.launch.py`, or simply launch
`ign gazebo` normally instead of `ign gazebo -s` for headless mode) rather
than launching headless and attaching `ign gazebo -g` separately.

To view the robot visually on the current (headless-first) setup, open a
GUI window in a separate terminal:
